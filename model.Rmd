...
Here we train a neural network.
...
cv_variables <- list(
    n_hidden_layers = c(1),
    hidden_layer_nodes = seq(2, 3, 1),# seq(2, 8, 1),
    kfolds = 10
)
...
for(layers in cv_variables$n_hidden_layers){
    for(nodes in cv_variables$hidden_layer_nodes){
        ...
        for(k in 1:cv_variables$kfolds){
            ...
            split_data_cv <- random_split(trainset, (cv_variables$kfolds - 1)/cv_variables$kfolds)
...
plot_results <- melt(results_df, id=c("nodes", "layers"))
